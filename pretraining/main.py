...
from scripts.play_eval import eval_aliengo
...
#----------------------------------------load real data-------------------------------
    ...
    if not os.path.exists(buffer_save_path):
        ...
        #     #print(str(dataset_real.items()))
        #     for key, value in dataset_real.items():
        #         #print('key: '+str(key)+' shape: '+str(value.shape)+'\nvalue: '+str(value))
    ...
    print('state_dim: {}, action_dim: {}'.format(state_dim, action_dim)) # state_dim is 290 (58 x 5), 5 is history_len, i.e., last 5 observations
    ...
    aliengo_eval = eval_aliengo(headless=False)
    aliengo_eval.prove_real_data(replay_buffer)
    
    # bc training
    # Behavioral Cloning (BC) implementation, a supervised learning technique 
    # used in offline reinforcement learning (RL) to train a policy by mimicking expert demonstrations.
        ...
        if not os.path.exists(best_bc_path):
            ...
            for step in tqdm(range(int(args.bc_steps)), desc='bc updating ......'):
                ...
                if step % int(args.log_freq) == 0:
                    ...
                    current_bc_score = aliengo_eval.play_aliengo(ensemble_bc.ensemble, torch.FloatTensor(mean).to(device), torch.FloatTensor(std).to(device))
    ...
    best_bppo_scores =  aliengo_eval.play_aliengo(abppo.bppo_ensemble, torch.FloatTensor(mean).to(device), torch.FloatTensor(std).to(device), 2)
    ...
    for step in tqdm(range(int(args.bppo_steps)), desc='bppo updating ......'):
        ...
        if (step+1) % 500 == 0:
            current_bppo_scores =  aliengo_eval.play_aliengo(abppo.bppo_ensemble, torch.FloatTensor(mean).to(device), torch.FloatTensor(std).to(device), 2)
