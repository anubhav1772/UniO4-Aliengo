...
constexpr uint16_t LOCAL_PORT = 8083;
...
{
public:
    Lcm_Server_Low(LeggedType rname) : udp(LOCAL_PORT, TARGET_IP,TARGET_PORT, sizeof(LowCmd), sizeof(LowState)), mylcm(LOWLEVEL){
        udp.InitCmdData(cmd);
    }
    void UDPRecv(){
        udp.Recv();
...
{
public:
    Lcm_Server_High(LeggedType rname): udp(LOCAL_PORT, TARGET_IP,TARGET_PORT, sizeof(HighCmd), sizeof(HighState)), mylcm(HIGHLEVEL){
        udp.InitCmdData(cmd);
    }
    void UDPRecv(){
        udp.Recv();
